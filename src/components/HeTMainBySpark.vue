...
import {
  ...
  APISecret,
  domain,
} from "@/hooks/getAuthorization";
import type { AIType, UserType, messageType } from "@/types";
import { emitter } from "@/utils/emitter";
import { decodeUnicode, md } from "@/utils/markdownUtils";
...
const sendMsg = async () => {
  ...
  socket.addEventListener("message", (event) => {
    ...
    replyContent.value += decodeUnicode(str);
      ...
      >
        <div class="chat-content">
          ...
          <div class="chat-ai" v-if="isLoading">
            <div class="chat-text" v-html="md.render(replyContent)"></div>
            <div class="info-time">
              <svg class="icon" aria-hidden="true">
                <use xlink:href="#icon-robot"></use>
              </svg>
